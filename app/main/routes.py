...
try:
    from ..proxmox.client import verify_proxmox_credentials, ProxmoxConnectionError, get_proxmox_vms
except ImportError:
...
def select_vm():
    """AJAX endpoint: update selected VM serials (multiple) in config.json.

    Accepts JSON { serials: ["1", "3", "5"] } or { serial: "3" } for backwards compatibility.
    """
    vms = session.get('last_vm_list')
    if not vms:
    ...
    if request.is_json:
        ...
        # Support both "serials" (array) and "serial" (single value) for backwards compatibility
        serials = body.get('serials') or body.get('serial')
    else:
        serials = request.form.get('serials') or request.form.get('serial') or request.form.get('selected_vm')
    ...
    try:
        ...
        serial_numbers = []
        for s in serials:
            serial_num = int(str(s).strip())
            if serial_num < 1 or serial_num > len(vms):
                return jsonify(success=False, message=f'Invalid serial number: {serial_num}. Valid range 1-{len(vms)}'), 400
            serial_numbers.append(serial_num)
    except ValueError as e:
        return jsonify(success=False, message=f'Invalid serial number format: {e}'), 400

    if not serial_numbers:
        return jsonify(success=False, message='No VMs selected'), 400

    # Update config via migration helper with all selected VMs
    if update_selected_vms:
        try:
            update_selected_vms(sorted(serial_numbers))
        except Exception as e:
            return jsonify(success=False, message=f'Failed to update config: {e}'), 500
        return jsonify(success=True, message='Selections saved', serials=sorted(serial_numbers))
...
def readiness_check():
    """Perform a real pre-migration readiness check using live source/destination data.

    The route SSHs into the destination host and compares available resources to
    the summed requirements of the selected VMs. No AI models are invoked.
    """
    vms = session.get('last_vm_list')
    dest_host = session.get('destination_host')
    ...
    for vm in selected_vms:
        try:
            ...
            pass
        try:
            total_ram_mb += int(vm.get('memoryMB', 0) or 0)
        except Exception:
            pass
    ...
    try:
        ...
        return jsonify(success=False, message='paramiko not installed on server'), 500
    ...
    try:
        ...
        return jsonify(success=False, message=f'Could not SSH to destination: {e}'), 500

    def run(cmd):
        stdin, stdout, stderr = client.exec_command(cmd)
        return stdout.read().decode().strip(), stderr.read().decode().strip()

    out, err = run("df -BG --output=avail / | tail -1")
    avail_disk = 0.0
    try:
        if out:
            avail_disk = float(out.strip().rstrip('G'))
    except Exception:
        pass

    out, err = run("free -m | awk '/Mem:/ {print $7}'")
    avail_mem_mb = 0
    try:
        avail_mem_mb = int(out.strip())
    except Exception:
        pass
...
@bp.route('/post-migration-check/<job_id>', methods=['GET'])
def post_migration_check(job_id):
    """Show post-migration check page comparing source and destination VM parameters."""
    import sys
    
    print(f"[ROUTE] /post-migration-check/{job_id} called", file=sys.stderr)
    
    # Get source VMs from session
    source_vms = session.get('last_vm_list')
    if not source_vms:
        flash("No source VM data found. Please start from the beginning.", "error")
        return redirect(url_for("main.index"))
    
    # Get destination details from session
    dest_host = session.get('destination_host')
    dest_user = session.get('destination_user')
    dest_pass = session.get('destination_pass')
    ...
    for source_vm in selected_source_vms:
        ...
        comparison = {
            ...
            'destination': {
                ...
                'ram_mb': matched_dest_vm.get('memory', 0) if matched_dest_vm else 0,
                'storage_gb': matched_dest_vm.get('total_storage_gb', 0) if matched_dest_vm else 0,
                'network': matched_dest_vm.get('network', []) if matched_dest_vm else [],
                'scsi_controller': matched_dest_vm.get('scsi_controller', 'none') if matched_dest_vm else 'none'
            } if matched_dest_vm else None,
            'matched': matched_dest_vm is not None
        }
        vm_comparisons.append(comparison)
